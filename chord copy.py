...
class ChordBase(Vertical):
   ...
   def get_string_tops(self) -> str:
      ...
      if position <= SECOND_FRET:  # Show double lines for open position or first/second fret
         s = " ╒═"
   ...
   def get_fret_row(self, row: int) -> str:
      if row == FRET_COUNT - 1:   
         ...
         if self.show_pos:
            ...
            base_fret = max(1, max_fret - FRET_OFFSET)  # Show at most 2 frets before the highest barre
